    ...
    for i in range(1,5)
        def build_model():
          ...
          model = keras.Sequential([
            h1=layers.Dense(NN, activation=tf.nn.relu, input_shape=[len(train_dataset.keys())]),
            h2=layers.Dense(NN, activation=tf.nn.relu),
            layers.Dense(NN, activation=tf.nn.relu),
            layers.Dense(1)
          ])
